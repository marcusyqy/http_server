...
static struct Da_Header *base_dynamic_array_get_header(void *ptr) {
  assert(ptr);
...
void *base_dynamic_array_reserve(void *ptr, size_t object_size, size_t capacity) {
  struct Da_Header *header = base_dynamic_array_get_header(ptr);
  if(capacity != 0) {
...
void base_dynamic_array_free(void *ptr) {
  free(base_dynamic_array_get_header(ptr));
}
...
size_t base_dynamic_array_capacity(void *ptr) {
  return base_dynamic_array_get_header(ptr)->capacity;
}
